...
Releasing is **tag-and-forget**. Pushing a tag like `v0.1.0` runs
`.github/workflows/publish.yml`, which derives the version from the tag
(`setuptools-scm`), publishes to PyPI via **Trusted Publishing (OIDC)** — no
API tokens stored — and creates the GitHub Release with autogenerated notes and
